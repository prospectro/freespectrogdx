...
object Antimancer {
  ...
  val bombardier = new Creature("antimancer.8.name", Attack(5), 49,
  ...
  val Antimancer: House = House("antimancer", List(
    ...
    Spell("antimancer.9.name", 
      ...
      inputSpec = Some(SelectTarget(cost96OrInf)),
  ...
  def cost96OrInf(p: PlayerId, state: GameState): List[Int] = {
    state.players(p).slots.foldLeft(List.empty[Int]) {
      case (acc, (i, s)) ⇒
  ...
  class HarvesterReaction extends Reaction {
    ...
    final override def onDeath(dead: Dead) : Unit = {
	  selected.otherPlayer.houses.decrMana(1, dead.card.houseIndex)
	  selected.focus()
    }
  }
